...
function toDropRate(d: DropRateRow): OrbDropRate {
	return { id: d.id, orb_id: d.orb_id, creature: d.creature, dungeon: d.dungeon, floor: d.floor, favorable_outcomes: d.favorable_outcomes, total_events: d.total_events, citation: cite(d) };
}
...
export async function getOrbs(db: D1Database): Promise<Orb[]> {
	const [orbRes, rateRes] = await db.batch([
		...
		db.prepare('SELECT * FROM orb_drop_rates ORDER BY orb_id'),
	]);
...
export async function getOrbBySlug(db: D1Database, slug: string): Promise<Orb | null> {
	...
	const rateRes = await db.prepare('SELECT * FROM orb_drop_rates WHERE orb_id = ?').bind(row.id).all<DropRateRow>();
